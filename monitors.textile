Monitoring Component Eventsp. A ComponentMonitor if injected into DefultPicoContainer allows for certain events to be monitored for the whole container. These are:
...
MutablePicoContainer pico = new DefaultPicoContainer(new ComposingMonitor( /* varargs of composer instances */));p. If more than one is specified in the varargs above, then the first in the list to actually return non-null will be used. It is also possible for a fallback component monitor to be specified:

ComponentMonitor otherCM = ... MutablePicoContainer pico = new DefaultPicoContainer(new ComposingMonitor(otherCM, /* varargs of composer instances */));p. See "Regex Composer":regex.html 
